...
async def test_full_auth_and_profile_flow():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        # 5. Earn Coins
        response = await client.post("/wallet/earn?minutes=10", headers=headers)
        assert response.status_code == 200, f"Earn failed: {response.text}"
        assert response.json()["balance"] == 110 # 100 base + 10% bonus for default 5.0 rating
        
        # 6. Withdraw Request - Should FAIL (No Stripe ID linked)
        response = await client.post(
            "/wallet/withdraw", 
        ...
        # 7. Manually link Stripe Account in DB
        ...
        # 8. Withdraw Request - SUCCESS (Mocked Stripe Transfer)
        ...
        with patch('app.services.payment_service.stripe.Transfer.create', return_value=mock_transfer):
            ...
            assert response.json()["remaining_balance"] == 10 # 110 - 100 withdrawn
